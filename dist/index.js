...
function ensureRefsAvailable(baseRef, headRef) {
  for (const ref of [baseRef, headRef]) {
    try {
      (0, import_node_child_process.execFileSync)("git", ["cat-file", "-e", ref], { stdio: "ignore" });
    } catch {
      try {
        (0, import_node_child_process.execFileSync)("git", ["fetch", "--depth=100", "origin", ref], { stdio: "ignore" });
      } catch {
      }
    }
  }
...
function getChangedFiles(baseRef, headRef) {
  ...
  const output = (0, import_node_child_process.execFileSync)("git", ["diff", "--name-only", "--diff-filter=ACMR", `${baseRef}...${headRef}`], {
    encoding: "utf8"
  });
  return output.split("\n").map((line) => line.trim()).filter(Boolean);
}

// src/dataKitParser.ts
var import_node_fs = require("node:fs");
var import_fast_xml_parser = __toESM(require_fxp());
var DATA_KIT_FILE_PATTERN = /dataPackageKitDefinitions\/[^/]+\.dataPackageKitDefinition-meta\.xml$/;
function isDataKitDefinitionFile(filePath) {
  return DATA_KIT_FILE_PATTERN.test(filePath);
}
function filterDataKitFiles(filePaths) {
  return filePaths.filter(isDataKitDefinitionFile);
}
var parser = new import_fast_xml_parser.XMLParser({ ignoreAttributes: true, trimValues: true });
function parseDataKitXml(filePath, xmlContent) {
  ...
  const rawDeploymentOrder = definition.deploymentOrder;
  const base = {
    filePath,
  ...
  try {
    ...
    throw new Error(`${filePath}: failed to parse deploymentOrder JSON - ${error.message}`);
  }
  return {
    ...base,
...
function parseDataKitFile(filePath) {
  return parseDataKitXml(filePath, (0, import_node_fs.readFileSync)(filePath, "utf8"));
}

// src/commentBuilder.ts
var COMMENT_MARKER = "<!-- sf-datapackagekit-pr-notifier -->";
function buildComment(kits) {
  const sections = kits.map((kit) => {
    const rows = kit.sequence.map((item, index) => `| ${index + 1} | ${item.devName} | ${item.type} | ${item.id ?? ""} |`).join("\n");
    return [
...
function hasCommentMarker(body) {
...
function loadEvent() {
  ...
  if (!eventPath) return {};
  return JSON.parse((0, import_node_fs2.readFileSync)(eventPath, "utf8"));
}
function resolvePullRequestRefs() {
...
async function findExistingCommentId(repo, prNumber, token) {
